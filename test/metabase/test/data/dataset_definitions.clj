(ns metabase.test.data.dataset-definitions
  ...
  (:require [clojure.tools.reader.edn :as edn]
            [metabase.test.data.interface :refer [def-database-definition update-table-def]])
...
(def-database-definition test-data-with-time
  (update-table-def "users"
                    (fn [table-def]
                      [(first table-def)
                    ...
                    (fn [rows]
                      (mapv (fn [[username last-login password-text]]
                            ...
                            rows))
                    (edn/read-string (slurp (str edn-definitions-dir "test-data" ".edn")))))
