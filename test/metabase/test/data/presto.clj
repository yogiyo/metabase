(ns metabase.test.data.presto
  (:require [clojure.string :as s]
            ...
            [metabase.driver.presto :as presto-driver]
...
(defn- qualify+quote-name [& names]
  (apply #'presto-driver/quote+combine-names (apply qualify-name names)))
...
(defn- field-base-type->dummy-value [field-type]
  ...
  (if (keyword? field-type)
    ...
    (field-base-type->dummy-value (#'presto-driver/presto-type->base-type field-type))))
...
(defn- create-table-sql [{:keys [database-name]} {:keys [table-name], :as tabledef}]
  (let [field-definitions (conj (:field-definitions tabledef) {:field-name "id", :base-type  :type/Integer})
    ...
    (format "CREATE TABLE %s AS SELECT * FROM (VALUES (%s)) AS t (%s) WHERE 1 = 0"
            ...
            (s/join \, dummy-values)
            (s/join \, (map #'presto-driver/quote-name columns)))))
...
(defn- create-db! [{:keys [table-definitions] :as dbdef}]
  (let [details (database->connection-details :db dbdef)]
    (doseq [tabledef table-definitions
      ...
      (#'presto-driver/execute-presto-query! details (drop-table-if-exists-sql dbdef tabledef))
      (#'presto-driver/execute-presto-query! details (create-table-sql dbdef tabledef))
      (doseq [batch batches]
        (#'presto-driver/execute-presto-query! details (insert-sql dbdef tabledef batch))))))
